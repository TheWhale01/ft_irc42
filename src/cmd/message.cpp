...
bool privmsg(Client &client, Server &serv, std::vector<std::string> const &args)
{
	...
	if (args[0][1] == '#' || args[0][1] == '&')
		send_to_members_in_chan(client, search_channel(client, args[0], serv.getChannels()), client.getNickName(), args[1]);
	else
	{
		std::cout << args[1];
		send_to_user(search_client(client, args[0], serv.getClients()), args[1]);
	}
	return (1);
}
